...
export default function AdminDepositsPage() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      setTransactions(data.filter((t: Transaction) => t.type === "DEPOSIT"));
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-navy-400">Review and approve user deposit claims.</p>
      </div>
            ...
            <tbody className="divide-y divide-navy-700">
              {transactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-navy-750 transition-colors">
                  ...
                  <td className="p-4 text-emerald-400 font-bold">${tx.amount.toLocaleString("en-US", { minimumFractionDigits: 2 })}</td>
              ...
              {transactions.length === 0 && (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-navy-400">No deposit requests found.</td>
                </tr>
              )}
            </tbody>
          </table>
